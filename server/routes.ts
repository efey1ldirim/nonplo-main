...
// Social media account validation service
const validateSocialMediaAccount = async (url: string, platform: string): Promise<boolean> => {
  try {
    ...
    const usernamePatterns: Record<string, RegExp> = {
      instagram: /^instagram\.com\/([a-zA-Z0-9_.]{1,30})$/,
      facebook: /^facebook\.com\/([a-zA-Z0-9_.]{1,50})$/,
      twitter: /^(twitter\.com|x\.com)\/([a-zA-Z0-9_]{1,15})$/,
      tiktok: /^tiktok\.com\/@([a-zA-Z0-9_.]{1,24})$/,
      youtube: /^youtube\.com\/(c\/|channel\/|user\/|@)?([a-zA-Z0-9_-]{1,100})$/,
      linkedin: /^linkedin\.com\/(company|in)\/([a-zA-Z0-9-]{1,100})$/
    };
    ...
    // For basic validation, we'll rely on format checking
    // Real validation would require API keys or scraping
    const match = path.match(pattern);
    if (!match) return false;
    
    const username = match[match.length - 1]; // Get the username part
    ...
    const inappropriateWords = [
      'yarrak', 'sik', 'amk', 'orospu', 'fuck', 'shit', 'dick', 'cock',
      'pussy', 'bitch', 'damn', 'hell'
    ];
